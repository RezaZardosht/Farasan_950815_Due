...
mbus_frame_free(mbus_frame *frame) {
    if (frame) {
        if (frame->next != NULL)
            mbus_frame_free(reinterpret_cast<mbus_frame *>(frame->next));
...
//------------------------------------------------------------------------------
int
mbus_data_bcd_encode(uint8_t *bcd_data, size_t bcd_data_size, int value) {
    int v0, v1, v2, x1, x2;
    size_t i;
...
//------------------------------------------------------------------------------
long long
mbus_data_bcd_decode(uint8_t *bcd_data, size_t bcd_data_size) {
    long long val = 0;
    size_t i;
...
//------------------------------------------------------------------------------
int
mbus_data_int_decode(uint8_t *int_data, size_t int_data_size) {
    int val = 0;
    size_t i;
...
long
mbus_data_long_decode(uint8_t *int_data, size_t int_data_size) {
    long val = 0;
    size_t i;
...
long long
mbus_data_long_long_decode(uint8_t *int_data, size_t int_data_size) {
    long long val = 0;
    size_t i;
...
//------------------------------------------------------------------------------
int
mbus_data_int_encode(uint8_t *int_data, size_t int_data_size, int value) {
    int i;
...
//------------------------------------------------------------------------------
float
mbus_data_float_decode(uint8_t *float_data) {
    float val = 0.0f;
...
//------------------------------------------------------------------------------
void
mbus_data_str_decode(uint8_t *dst, const uint8_t *src, size_t len) {
    size_t i;
...
//------------------------------------------------------------------------------
void
mbus_data_bin_decode(uint8_t *dst, const uint8_t *src, size_t len, size_t max_len) {
    size_t i, pos;
    ...
    if (src && dst) {
        while ((i < len) && ((pos + 3) < max_len)) {
            pos += snprintf(reinterpret_cast<char *>(&dst[pos]), max_len - pos, "%.2X ", src[i]);
            i++;
        }
...
void mbus_data_tm_decode(struct tm_Mbus *t, uint8_t *t_data, size_t t_data_size) {
    if (t && t_data) {
        t->tm_sec = 0;
        t->tm_min = 0;
        t->tm_hour = 0;
...
//------------------------------------------------------------------------------
int
mbus_data_manufacturer_encode(uint8_t *m_data, uint8_t *m_code) {
    int m_val;
...
mbus_decode_manufacturer(uint8_t byte1, uint8_t byte2) {
    ...
    m_str[0] = (uint8_t) (((m_id >> 10) & 0x001F) + 64);
    m_str[1] = (uint8_t) (((m_id >> 5) & 0x001F) + 64);
    m_str[2] = (uint8_t) (((m_id) & 0x001F) + 64);
    m_str[3] = 0;
...
///
const char *
mbus_data_variable_medium_lookup(uint8_t medium) {
    static char buff[256];
...
//------------------------------------------------------------------------------
uint8_t
mbus_dif_datalength_lookup(uint8_t dif) {
    switch (dif & 0x0F) {
        case 0x0:
            return 0;
        case 0x1:
...
//------------------------------------------------------------------------------
const char *
mbus_vif_unit_lookup(uint8_t vif) {
    static char buff[256];
    int n;
    ...
    if (record) {
        ...
        switch (record->drh.dib.dif & 0x0F) {
            ...
            case 0x0F: // special functions
                ...
                mbus_data_bin_decode(reinterpret_cast<uint8_t *>( buff), record->data, record->data_len, sizeof(buff));
                break;

            case 0x0D: // variable length
                if (record->data_len <= 0xBF) {
                    mbus_data_str_decode(reinterpret_cast<uint8_t *>( buff), record->data, record->data_len);
                    break;
                }
                /* FALLTHROUGH */
...
mbus_Ndata_record *Mbus_LastListRecord(mbus_Ndata_record *record) {
    mbus_Ndata_record *Newrecord = record;
...
int add_NrecordPackFrame(mbus_frame *replay_frame, mbus_Ndata_record *Nrecord) {
    ...
    for (int j = 0; j < Nrecord->drh.dib.ndife; j++) {
        replay_frame->data[replay_frame->data_size++] = Nrecord->drh.dib.dife[j];
    }
...
//------------------------------------------------------------------------------
int
mbus_parse(uint8_t *data, size_t data_size) {
    size_t i, len;
    char Tmsg[100];
    ...
    if (data && data_size > 0) {
        ...
        switch (data[0]) {
            ...
            case MBUS_FRAME_SHORT_START:
                ...
                if (frame->control == MBUS_CONTROL_MASK_SND_NKE) // initialize Slave by master
                {
                    mbus_frame *replay_frame;
...
mbus_data_variable_parse(mbus_frame *frame, mbus_data_variable *data) {
    ...
    if (frame && data) {
        ...
        while (i < frame->data_size) {
            ...
            // copy timestamp
            memcpy((void *) &(record->timestamp), (void *) &(frame->timestamp), sizeof(long));
...
mbus_frame_print(mbus_frame *frame) {
    ...
    for (iter = frame; iter; iter = reinterpret_cast<mbus_frame *>( iter->next)) {
...
//------------------------------------------------------------------------------
void
mbus_str_xml_encode(char *dst, const char *src, size_t max_len) {
    mbus_str_xml_encode(reinterpret_cast<uint8_t *>(dst), reinterpret_cast<const uint8_t *>(src), max_len);
}
...
mbus_str_xml_encode(uint8_t *dst, const uint8_t *src, size_t max_len) {
    ...
    if (src != NULL) {
        while ((len + 6) < max_len) {
            ...
            switch (src[i]) {
                case '&':
                    len += snprintf(reinterpret_cast<char *>(&dst[len]), max_len - len, "&amp;");
                    break;
                case '<':
                    len += snprintf(reinterpret_cast<char *>(&dst[len]), max_len - len, "&lt;");
                    break;
...
//------------------------------------------------------------------------------
mbus_Ndata_record *
mbus_Ndata_record_new() {
    mbus_Ndata_record *record;

    if ((record = (mbus_Ndata_record *) malloc(sizeof(mbus_Ndata_record))) == NULL) {
        return NULL;
    }
...
//------------------------------------------------------------------------------
void
mbus_Ndata_record_free(mbus_Ndata_record *record) {
    if (record) {
        mbus_Ndata_record *next = record->next;

        free(record);

        if (next)
            mbus_Ndata_record_free(next);
    }
}
