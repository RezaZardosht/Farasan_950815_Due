...
typedef struct _mbus_frame {
  ...
  _mbus_frame *next; // pointer to next mbus_frame for multi-telegram replies
...
typedef struct _mbus_data_record {
  ...
  _mbus_data_record *next;

} mbus_data_record;

//////////////alaki
typedef struct _mbus_Ndata_record {
...
#define MBUS_CONTROL_MASK_SND_UD        0x73
#define MBUS_CONTROL_MASK_REQ_UD2       0x7B
#define MBUS_CONTROL_MASK_REQ_UD1       0x7A
...
// variable length records
//
mbus_Ndata_record *mbus_Ndata_record_new();
...
// Parse/Pack to bin
//
int mbus_parse( uint8_t *data, size_t data_size);
...
int mbus_frame_pack(mbus_frame *frame, uint8_t *data, size_t data_size);
...
//
void  mbus_str_xml_encode(char *dst, const char *src, size_t max_len);
...
int mbus_data_int_encode(uint8_t *int_data, size_t int_data_size, int value);

long long mbus_data_bcd_decode(uint8_t *bcd_data, size_t bcd_data_size);
int  mbus_data_int_decode(uint8_t *int_data, size_t int_data_size);
long mbus_data_long_decode(uint8_t *int_data, size_t int_data_size);
long long mbus_data_long_long_decode(uint8_t *int_data, size_t int_data_size);
