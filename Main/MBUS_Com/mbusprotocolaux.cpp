...
#define MBUS_ERROR(x) IF_SERIAL_DEBUG(ardprintf(( x )))
#define MBUS_ERROR1(x, y) IF_SERIAL_DEBUG(ardprintf(( x ),y))
#define MBUS_ERROR2(x,y,z) IF_SERIAL_DEBUG(ardprintf(( x ),y,z))
#define MBUS_ERROR3(x,y,z,t) IF_SERIAL_DEBUG(ardprintf(( x ),y,z,t))

#ifdef _DEBUG_
#define MBUS_DEBUG(x) IF_SERIAL_DEBUG(ardprintf(( x )))
#define MBUS_DEBUG1(x, y) IF_SERIAL_DEBUG(ardprintf(( x ),y))
#define MBUS_DEBUG2(x,y,z) IF_SERIAL_DEBUG(ardprintf(( x ),y,z))
#else
#define MBUS_DEBUG(...)
#endif
...
mbus_connect_serial(HardwareSerial *device)
{
  ...
  {
    IF_SERIAL_DEBUG(ardprintf(("mbus_connect_serial: Failed to allocate handle.\n")));
    return NULL;
  }
...
mbus_sendrecv_request(mbus_handle *handle, int address, mbus_frame *reply, int max_frames)
{
  ...
  if (debug)
    ardprintf(("%s: debug: sending request frame\n"), "mbus_sendrecv_request");
  ...
  while (more_frames)
  {
    ...
    if (debug)
      ardprintf(("%s: debug: receiving response frame #%d\n"), "mbus_sendrecv_request", frame_count);
    ...
    else
    {
      ...
      {
        if (debug)
          ardprintf(("%s: debug: expecting more frames\n"), "mbus_sendrecv_request");
        ...
        if (debug)
          ardprintf(("%s: debug: resending request frame\n"), "mbus_sendrecv_request");
      ...
      {
        if (debug)
          ardprintf(("%s: debug: no more frames\n"), "mbus_sendrecv_request");
      }
    }
...
mbus_send_ping_frame(mbus_handle *handle, int address)
{
  ...
  IF_SERIAL_DEBUG(ardprintf(("mbus_send_ping_frame Entered\n")));
...
mbus_scan_2nd_address_range(mbus_handle * handle, int pos, char *addr_mask)
{
  ...
  for (i = 0; i <= 9; i++)
  {
    ...
    if (probe_ret == MBUS_PROBE_SINGLE)
    {
      if (!_mbus_found_event)
      {
        ardprintf(("Found a device on secondary address %s [using address mask %s]\n"), matching_mask, mask);
      }
    }
    ...
    {
      ardprintf(("%s: Failed to probe secondary address [%s].\n"), __PRETTY_FUNCTION__, mask);
      return -1;
    }
  }
