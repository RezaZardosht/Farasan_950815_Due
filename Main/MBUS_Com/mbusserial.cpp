...
mbus_serial_send_frame(mbus_frame *frame) {
    ...
    if ((ret = SerialMBUS.write(buff, len)) == len) {
        ...
        IF_SERIAL_DEBUG(
                printf_New("%s: Failed to write frame to socket (ret = %d: )\n", "mbus_serial_send_frame", ret));
...
//    IF_SERIAL_DEBUG(printf_New("%s: Entered3 \n", __PRETTY_FUNCTION__));
    ...
    int iecChar;
    while (SerialMBUS.available() > 0 && PermitContinue == false) {
        iecChar = SerialMBUS.read();
            if (iecChar == MBUS_FRAME_ACK_START || iecChar == MBUS_FRAME_SHORT_START || iecChar == MBUS_FRAME_LONG_START ) {
                PermitContinue = true;
            }
        else
        if (iecChar != 0 && iecChar < 200)
            PermitContinue = true;
    }
