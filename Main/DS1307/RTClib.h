...
class DateTime_RTC {
public:
    DateTime_RTC (uint32_t t =0);
    DateTime_RTC (uint16_t year, uint8_t month, uint8_t day,
                uint8_t hour =0, uint8_t min =0, uint8_t sec =0);
    DateTime_RTC (const DateTime_RTC& copy);
    DateTime_RTC (const char* date, const char* time);
    DateTime_RTC (const __FlashStringHelper* date, const __FlashStringHelper* time);
    ...
    DateTime_RTC operator+(const TimeSpan& span);
    DateTime_RTC operator-(const TimeSpan& span);
    TimeSpan operator-(const DateTime_RTC& right);
...
public:
    boolean begin(void);
    static void adjust(const DateTime_RTC& dt);
    uint8_t isrunning(void);
    static DateTime_RTC now();
...
public:
    boolean begin(void);
    void adjust(const DateTime_RTC& dt);
...
class RTC_Millis {
public:
    static void begin(const DateTime_RTC& dt) { adjust(dt); }
    static void adjust(const DateTime_RTC& dt);
    static DateTime_RTC now();
